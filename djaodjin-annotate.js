    ...
    Annotate.prototype = {
        init: function() {
            ...
            if (self.options.storedElement) {
                self.storedElement = self.options.storedElement;
            }
            ...
            self.baseLayerId = 'baseLayer_' + self.myCanvasContainerId;
            self.drawingLayerId = 'drawingLayer_' + self.myCanvasContainerId;
            self.toolOptionId = 'tool_option_' + self.myCanvasContainerId;
            ...
            if (self.options.useOwnTemplate) {
                self.$tool = $('#' + self.options.toolboxId);
            } else if (self.options.bootstrap) {
            ...
            self.checkUndoRedo();
            var toolbox$ = $('#' + self.options.toolboxId);
            toolbox$.css({
                cursor: 'pointer',
            });
        ...
        startEditing: function() {
            var self = this;
            self.isToolboxDisable = !self.isToolboxDisable;
            self.$el.attr('disabled', self.isToolboxDisable);
            var toolbox$ = $('#' + self.options.toolboxId);
        ...
        stopEditing: function() {
            ...
            self.isToolboxDisable = !self.isToolboxDisable;
            self.$el.attr('disabled', self.isToolboxDisable);
            if (self.isToolboxDisable && self.options.onSaveAnnotations && self.storedElement.length > 0) {
                self.options.onSaveAnnotations(self);
            }
        },
        ...
        undoaction: function(event) {
            ...
            var lastElement = self.storedElement[self.storedElement.length -
                1];

            self.storedUndo.push(lastElement);
        ...
        redoaction: function(event) {
            ...
            var lastElement = self.storedUndo[self.storedUndo.length - 1];
            self.storedElement.push(lastElement);
            self.storedUndo.pop();
            self.checkUndoRedo();
            self.clear();
            self.redraw();
